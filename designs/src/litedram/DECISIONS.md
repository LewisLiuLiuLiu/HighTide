...
`litedram_core.v` is **no longer a checked-in vendored file** regenerated by a `dev/setup.sh` venv + git submodule. It is generated hermetically by Bazel:

- **migen / litex / litedram** are fetched as pinned source `http_archive`s (`@migen_src`, `@litex_src`, `@litedram_src` in the root `MODULE.bazel`) and exposed as `py_library` via injected `{migen,litex,litedram}.BUILD.bazel`. Their PyPI leaf deps (`pyyaml`, `pyserial`, `requests`, `packaging`, `colorama`) come from a dedicated `pip.parse(hub_name = "pip_litex")` hub against `requirements_lock.txt` — a per-tool-family hub so other generators can pin different versions side-by-side.
- The `:litedram_gen` `py_binary` (wrapper `litedram_gen_main.py` → `runpy` of upstream `litedram/gen.py`) and the `:gen_litedram_core` genrule replace `dev/setup.sh` + `gen.sh`. No venv, no submodule, no build-time `pip install`.
- **Reproducibility**: LiteX stamps two build timestamps (`// Date` header + `// Auto-Generated by LiteX` footer) and orders the blackbox listing in the header comment via Python set iteration. The genrule runs with `PYTHONHASHSEED=0` (stabilizes the listing) and `sed`-normalizes both timestamp lines, so the output is byte-reproducible for deterministic Bazel caching. The RTL body is identical to the previously-committed core.
- **The `sdram_dq` `input`→`inout` fix** (LiteX GENSDRPHY declares the bidirectional SDR DQ bus as `input`) is now a declarative `patches/litedram_core_sdram_dq_inout.patch` applied in the genrule (was a `patch/` file applied by the retired `gen.sh`).
- `libraries/lattice/*.v` (the four ECP5 I/O primitive stubs) stay checked in — they are HighTide-authored, not upstream. Consumers still reference the stable `//designs/src/litedram:litedram_rtl` label.
